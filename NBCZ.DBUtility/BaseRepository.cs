    ...
    public class BaseRepository<T> : SimpleClient<T> where T : class,new()
    {
        ...
        public int Update(T entity, bool ignoreNullColumns = false, object data = null)
        {
            return Context.Updateable(entity).IgnoreColumns(ignoreNullColumns)
                .EnableDiffLogEventIF(data != null, data).ExecuteCommand();
        }
        ...
        {
            return Context.Deleteable<T>().In(obj).EnableDiffLogEventIF(title != null, title).ExecuteCommand();
        }
        public int Delete(object id, string title = "")
        {
            return Context.Deleteable<T>(id).EnableDiffLogEventIF(title != null, title).ExecuteCommand();
        }
