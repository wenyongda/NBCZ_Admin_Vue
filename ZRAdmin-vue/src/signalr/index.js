...
export default {
  ...
  init(url) {
    ...
    const connection = $.hubConnection(socketUrl, { useDefaultPath: false })
    const messageHub = connection.createHubProxy('messageHub')
    // const messageHub = connection.messageHub

    // messageHub.on('onlineNum', (data) => {
    //   // useSocketStore().setOnlineUserNum(data)
    // })
    console.log(messageHub)
    analysis.onMessage(messageHub)
